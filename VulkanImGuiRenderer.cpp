...
bool VulkanImGuiRenderer::Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
	...
	ImGui_ImplVulkan_Init(&m_Info, render_pass);
...
bool VulkanImGuiRenderer::CreateFontsTexture()
{
	...
	auto command_buffer = vkApi::VulkanCommandBuffer::beginSingleTimeCommands(true);
	ImGui_ImplVulkan_CreateFontsTexture((VkCommandBuffer)command_buffer);
	vkApi::VulkanCommandBuffer::flushSingleTimeCommands(command_buffer, true);
	vkApi::VulkanCore::Instance()->getDevice().waitIdle();
	return true;
}
...
vk::DescriptorSet VulkanImGuiRenderer::CreateImGuiTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout, vk::DescriptorSet* vExistingDescriptorSet)
{
	...
	if (!vExistingDescriptorSet ||
		(vExistingDescriptorSet && !*vExistingDescriptorSet))
	{
		// Create Descriptor Set:
		VkDescriptorSetAllocateInfo alloc_info = {};
		alloc_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
