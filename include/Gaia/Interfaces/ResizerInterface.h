...
class GAIA_API ResizerInterface {
protected:
	bool m_ResizingByResizeEventIsAllowed = false;
	bool m_ResizingByHandIsAllowed = false;

public:
	virtual void NeedResizeByHand(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers = nullptr) { UNUSED(vNewSize); UNUSED(vCountColorBuffers); }
	virtual void NeedResizeByResizeEvent(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers = nullptr) = 0;
