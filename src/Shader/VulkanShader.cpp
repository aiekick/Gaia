...
const std::vector<unsigned int> VulkanShader::CompileGLSLString(const std::string& vCode,
    ...
    // LogVarDebugInfo("Debug : ==== VulkanShader::CompileGLSLString (%s) =====", vShaderSuffix.c_str());
    ...
    if (SpirV.empty()) {
        ...
        // LogVarDebugInfo("Debug : Shader stage %s Spirv generation of %s : OK !", vShaderSuffix.c_str(), vOriginalFileName.c_str());
    }
...
vk::ShaderModule VulkanShader::CreateShaderModule(vk::Device vLogicalDevice, std::vector<unsigned int> vSPIRVCode) {
    ...
    if (!vSPIRVCode.empty()) {
        ...
        if (vLogicalDevice.createShaderModule(&createInfo, nullptr, &shaderModule) != vk::Result::eSuccess) {
            LogVarError("Debug : fail to create shader module !");
            return vk::ShaderModule{};
        }
    } else {
        LogVarError("Debug : SPIRV Code is empty. Fail to create shader module !");
        return vk::ShaderModule{};
    }

    if (shaderModule) {
        // LogVarDebugInfo("Debug : shader module compilation : OK !");
    }
