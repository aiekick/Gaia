...
void VulkanCommandBuffer::flushSingleTimeCommands(
    GaiApi::VulkanCoreWeak vVulkanCore, vk::CommandBuffer& commandBuffer, bool end, vk::CommandPool* vCommandPool) {
    ZoneScoped;
...
VulkanCommandBuffer VulkanCommandBuffer::CreateCommandBuffer(
    GaiApi::VulkanCoreWeak vVulkanCore, vk::QueueFlagBits vQueueType, vk::CommandPool* vCommandPool) {
    ZoneScoped;
    ...
    if (vQueueType == vk::QueueFlagBits::eGraphics) {
        ...
        if (vCommandPool)
            commandBuffer.commandpool = *vCommandPool;
        else
            commandBuffer.commandpool = graphicQueue.cmdPools;
        ...
        if (vCommandPool)
            ...
            commandBuffer.commandpool = computeQueue.cmdPools;
    }

    commandBuffer.cmd =
        device.allocateCommandBuffers(vk::CommandBufferAllocateInfo(commandBuffer.commandpool, vk::CommandBufferLevel::ePrimary, 1))[0];
