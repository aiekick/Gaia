...
bool ShaderPass::InitPixelWithoutFBO(const ez::uvec2& vSize,
...
bool ShaderPass::InitPixel(const ez::uvec2& vSize,
    const uint32_t& vCountColorBuffers,
...
bool ShaderPass::InitCompute2D(
    const ez::uvec2& vDispatchSize, const uint32_t& vCountColorBuffers, const bool& vPingPongBufferMode, const vk::Format& vFormat) {
    ZoneScoped;
...
bool ShaderPass::InitCompute3D(const ez::uvec3& vDispatchSize) {
    ZoneScoped;
...
bool ShaderPass::DrawWidgets(const uint32_t& vCurrentFrame, ImGuiContext* vContextPtr, void* vUserDatas) {
    ZoneScoped;
...
bool ShaderPass::DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContextPtr, void* vUserDatas) {
    ZoneScoped;
    UNUSED(vCurrentFrame);
    UNUSED(vRect);
    assert(vContextPtr);
...
void ShaderPass::SetRenderDocDebugName(const char* vLabel, ez::fvec4 vColor) {
    ZoneScoped;
#ifdef VULKAN_DEBUG
...
void ShaderPass::NeedResizeByResizeEvent(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers) {
    ZoneScoped;
...
bool ShaderPass::NeedResizeByResizeEventIfChanged(ez::ivec2 vNewSize) {
    ZoneScoped;
    // need resize ?
...
void ShaderPass::NeedResizeByHand(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers) {
    ZoneScoped;
...
bool ShaderPass::NeedResizeByHandIfChanged(ez::ivec2 vNewSize) {
    ZoneScoped;
    // need resize ?
    if (IS_FLOAT_DIFFERENT((float)vNewSize.x, m_OutputSize.x) && IS_FLOAT_DIFFERENT((float)vNewSize.y, m_OutputSize.y)) {
        // yes !
...
void ShaderPass::NeedResize(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers) {
    ZoneScoped;
...
void ShaderPass::Resize(const ez::uvec2& vNewSize) {
    ZoneScoped;
    ...
    if (m_ResizingByHandIsAllowed || m_ResizingByResizeEventIsAllowed) {
        ...
        m_OutputSize = ez::fvec2((float)vNewSize.x, (float)vNewSize.y);
...
void ShaderPass::AutoResizeBuffer(OutputSizeInterface* vBufferOutSizePtr, ez::fvec2* vParentOutSizePtr) {
    ZoneScoped;
    if (vBufferOutSizePtr && vParentOutSizePtr) {
        ...
        if (!vParentOutSizePtr->emptyOR() &&
            ...
            ez::ivec2 new_size = ez::ivec2((int32_t)vParentOutSizePtr->x, (int32_t)vParentOutSizePtr->y);
...
void ShaderPass::UpdatePixel2DViewportSize(const ez::uvec2& vNewSize) {
    ZoneScoped;
    if (m_ResizingByHandIsAllowed || m_ResizingByResizeEventIsAllowed) {
        m_RenderArea = vk::Rect2D(vk::Offset2D(), vk::Extent2D(vNewSize.x, vNewSize.y));
        m_Viewport = vk::Viewport(0.0f, 0.0f, static_cast<float>(vNewSize.x), static_cast<float>(vNewSize.y), 0, 1.0f);
...
ez::fvec2 ShaderPass::GetOutputSize() {
    return m_OutputSize;
}

ez::fvec2 ShaderPass::GetOutputRatio() {
...
void ShaderPass::SetLocalGroupSize(const ez::uvec3& vLocalGroupSize) {
    ZoneScoped;
    ...
    ez::uvec3 min = 1U;
    ez::uvec3 max = ez::uvec3(max_group[0], max_group[1], max_group[2]);
    m_LocalGroupSize = ez::clamp(vLocalGroupSize, min, max);
}

void ShaderPass::SetDispatchSize1D(const uint32_t& vDispatchSize) {
    ZoneScoped;

    SetDispatchSize3D(ez::uvec3(vDispatchSize, 1U, 1U));
}

void ShaderPass::SetDispatchSize2D(const ez::uvec2& vDispatchSize) {
    ZoneScoped;

    SetDispatchSize3D(ez::uvec3(vDispatchSize, 1U));
}

void ShaderPass::SetDispatchSize3D(const ez::uvec3& vDispatchSize) {
    ZoneScoped;
    ...
    ez::uvec3 min = 1U;
    ez::uvec3 max = ez::uvec3(max_dispatch[0], max_dispatch[1], max_dispatch[2]);
    m_DispatchSize = ez::clamp(m_DispatchSize, min, max);
}

const ez::uvec3& ShaderPass::GetDispatchSize() {
    ZoneScoped;
...
void ShaderPass::SetCountVertexs(const uint32_t& vCountVertexs) {
    ...
    m_CountVertexs.w = ez::maxi<uint32_t>(1u, vCountVertexs);
}

void ShaderPass::SetCountInstances(const uint32_t& vCountInstances) {
    ZoneScoped;

    m_CountInstances.w = ez::maxi<uint32_t>(1u, vCountInstances);
}

void ShaderPass::SetCountIterations(const uint32_t& vCountIterations) {
    ZoneScoped;

    m_CountIterations.w = ez::maxi<uint32_t>(1u, vCountIterations);
}
...
ShaderPass::ShaderCode ShaderPass::CompilShaderCode(
    ...
    if (shaderCode.m_Used) {
        if (!m_DontUseShaderFilesOnDisk) {
            ...
            auto shader_path =  shaderCode.m_FilePathName;
            if (ez::file::isFileExist(shader_path)) {
                shaderCode.m_Code = ez::file::loadFileToString(shader_path);
            } else {
                ez::file::saveStringToFile(shaderCode.m_Code, shader_path);
            }
        }
                ...
                if (shader.m_Used && !shader.m_FilePathName.empty()) {
                    for (auto filePathNameToUpdate : vFiles) {
                        if (filePathNameToUpdate == shader.m_FilePathName) {
                            needReCompil |= ez::file::isFileExist(shader.m_FilePathName);
                        }
                    }
                }
            }
    ...
    if (ImGui::CollapsingHeader("Resizing")) {
        ...
        if (m_ResizingByHandIsAllowed) {
            ...
            if (ImGui::SliderFloatDefaultCompact(aw, "Quality (smaller value is better)", &m_BufferQuality, 0.5f, 4.0f, 1.0f, 0.25f)) {
                m_BufferQuality = ez::clamp(m_BufferQuality, 0.25f, 4.0f);
                resize = true;
            }

            ez::uvec2 size = m_OutputSize;
            ...
            if (resize) {
                ez::fvec2 new_quality_size = ez::clamp(ez::fvec2(size) / m_BufferQuality, 1.0f, 4096.0f);  // 2048 / 0.5 => 4096
                ez::ivec2 new_size = ez::clamp(ez::ivec2(new_quality_size), 1, 2048);
