    ...
    if (vCountColorBuffers > 0 && vCountColorBuffers <= 8) {
        ...
        if (!size.emptyOR()) {
           ...
           // necessaire pour une renderpass compatible
            ...
            // pour le sampleCount
            attachments.resize(vCountColorBuffers + (useMultiSampling ? vCountColorBuffers : 0U) + (vUseDepth ? 1U : 0U));
            ...
            for (uint32_t j = 0; j < vCountColorBuffers; j++) {
                if (attachments[attIndex].InitColor2D(m_VulkanCore, size, format, 1U, vNeedToClear, sampleCount)) {
                    ...
                    if (vNeedToClear) {
                        clearColorValues.push_back(
                            vk::ClearColorValue(std::array<float, 4>{vClearColor.x, vClearColor.y, vClearColor.z, vClearColor.w}));
                    ...
                    LogVarDebugInfo("Debug : Color Attachement %u cant be loaded...", attIndex);
                    return false;
                }
                ++attIndex;
            }
            ...
            if (useMultiSampling) {
                for (uint32_t j = 0; j < vCountColorBuffers; j++) {
                    if (attachments[attIndex].InitColor2D(m_VulkanCore, size, format, 1U, vNeedToClear, vk::SampleCountFlagBits::e1)) {
                        ...
                        LogVarDebugInfo("Debug : Resolve Attachement %u cant be loaded...", attIndex);
                        return false;
                    }
                    ++attIndex;
                }
            ...
            if (vUseDepth) {
                ...
                if (attachments[attIndex].InitDepth(m_VulkanCore, size, vk::Format::eD32SfloatS8Uint, sampleCount)) {
                    ...
                    if (vNeedToClear) {
                        ...
                        attachmentClears.push_back(vk::ClearAttachment(
                            vk::ImageAspectFlagBits::eDepth | vk::ImageAspectFlagBits::eStencil, attIndex, clearColorValues[attIndex]));
                        rectClears.push_back(vk::ClearRect(vk::Rect2D(vk::Offset2D(), vk::Extent2D(width, height)), 0, 1));
                    }
                } else {
                    LogVarDebugInfo("Debug : Depth Attachement can't be loaded");
                    return false;
                }
                ++attIndex;
            }
            ...
            if (vCreateRenderPass) {
                ...
                std::vector<vk::SubpassDependency> dependencies;
                {
                    ...
                    /*{
                        vk::SubpassDependency selfDependencie;
                        selfDependencie.srcSubpass       = 0;
                        selfDependencie.dstSubpass       = 0;
                        selfDependencie.srcStageMask     = vk::PipelineStageFlagBits::eColorAttachmentOutput;
                        selfDependencie.srcAccessMask    = vk::AccessFlagBits::eColorAttachmentWrite;
                        selfDependencie.dstStageMask     = vk::PipelineStageFlagBits::eFragmentShader;
                        selfDependencie.dstAccessMask    = vk::AccessFlagBits::eShaderRead;
                        selfDependencie.dependencyFlags  = vk::DependencyFlagBits::eByRegion;
                        dependencies.push_back(selfDependencie);
                    }*/
                }
            ...
            LogVarDebugInfo("Debug : Size is empty on one chnannel at least : x:%u,y:%u", size.x, size.y);
        }
    } else {
