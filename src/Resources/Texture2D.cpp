...
bool Texture2D::loadImage(
    const std::string& inFile, std::vector<uint8_t>& outBuffer, uint32_t& outWidth, uint32_t& outHeight, uint32_t& outChannels) {
    ZoneScoped;
...
bool Texture2D::loadImageWithMaxH(const std::string& inFile, const uint32_t& maxHeight, uint32_t& outWidth, std::vector<uint8_t>& outBuffer) {
    ...
    if (datas) {
        if (w && h && chans) {
            ...
            const int resizeSucceeded = stbir_resize_uint8(datas, w, h, 0, resizedData, outWidth, newHeight, 0,
                4);  //-V112
...
/*
vk::DescriptorImageInfo Texture2D::GetImageInfoFromMemory(GaiApi::VulkanCoreWeak vVulkanCore, uint8_t* buffer, const uint32_t& width, const uint32_t&
height, const uint32_t& channels)
{
    ...
    if (buffer)
    {
        ...
        imgInfo.imageView = corePtr->getDevice().createImageView(imViewInfo);
        ...
        imgInfo.sampler = corePtr->getDevice().createSampler(samplerInfo);
...
Texture2DPtr Texture2D::CreateFromFile(GaiApi::VulkanCoreWeak vVulkanCore, std::string vFilePathName, const uint32_t& vMaxHeight) {
    ZoneScoped;
...
Texture2DPtr Texture2D::CreateFromMemory(
...
Texture2D::Texture2D(GaiApi::VulkanCoreWeak vVulkanCore) : m_VulkanCore(vVulkanCore) {
    ZoneScoped;
}
...
bool Texture2D::LoadMemory(uint8_t* buffer,
    const uint32_t& width,
    const uint32_t& height,
    const uint32_t& channels,
    const vk::Format& vFormat,
    const uint32_t& vMipLevelCount) {
    ZoneScoped;
...
bool Texture2D::LoadEmptyTexture(const ez::uvec2& vSize, const vk::Format& vFormat) {
    ...
    m_Texture2D = VulkanRessource::createTextureImage2D(m_VulkanCore, vSize.x, vSize.y, 1, vFormat, image_data.data(), "Texture2D");
...
bool Texture2D::LoadEmptyImage(const ez::uvec2& vSize, const vk::Format& vFormat) {
    ...
    m_Texture2D = VulkanRessource::createComputeTarget2D(m_VulkanCore, vSize.x, vSize.y, 1U, vFormat, vk::SampleCountFlagBits::e1, "Texture2D");
...
bool Texture2D::SaveToPng(const std::string& vFilePathName, const bool& vFlipY, const int& vSubSamplesCount, const ez::uvec2& vNewSize) {
    ...
    if (vNewSize.x != width || vNewSize.y != height) {
        ...
        int resizeRes = stbir_resize_uint8(
            bmBytesRGBA, width, height, width * bytesPerPixel, resizedData, newWidth, newHeight, newWidth * bytesPerPixel, bytesPerPixel);

        if (resizeRes) {
            if (vFlipY)
                stbi_flip_vertically_on_write(1);
            int resWrite = stbi_write_png(vFilePathName.c_str(), newWidth, newHeight, bytesPerPixel, resizedData, newWidth * bytesPerPixel);
        ...
        if (vFlipY)
            stbi_flip_vertically_on_write(1);
        int resWrite = stbi_write_png(vFilePathName.c_str(), width, height, bytesPerPixel, bmBytesRGBA, width * bytesPerPixel);
...
bool Texture2D::SaveToBmp(const std::string& vFilePathName, const bool& vFlipY, const int& vSubSamplesCount, const ez::uvec2& vNewSize) {
    ...
    if (vNewSize.x != width || vNewSize.y != height) {
        ...
        int resizeRes = stbir_resize_uint8(
            bmBytesRGB, width, height, width * bytesPerPixel, resizedData, newWidth, newHeight, newWidth * bytesPerPixel, bytesPerPixel);

        if (resizeRes) {
            if (vFlipY)
                stbi_flip_vertically_on_write(1);
            int resWrite = stbi_write_bmp(vFilePathName.c_str(), newWidth, newHeight, bytesPerPixel, resizedData);
        ...
        if (vFlipY)
            stbi_flip_vertically_on_write(1);
        int resWrite = stbi_write_bmp(vFilePathName.c_str(), width, height, bytesPerPixel, bmBytesRGB);
...
bool Texture2D::SaveToJpg(
    ...
    if (vNewSize.x != width || vNewSize.y != height) {
        ...
        if (resizeRes) {
            if (vFlipY)
                stbi_flip_vertically_on_write(1);
            int resWrite = stbi_write_jpg(vFilePathName.c_str(), newWidth, newHeight, bytesPerPixel, resizedData, vQualityFrom0To100);
        ...
        if (vFlipY)
            stbi_flip_vertically_on_write(1);
        int resWrite = stbi_write_jpg(vFilePathName.c_str(), width, height, bytesPerPixel, bmBytesRGB, vQualityFrom0To100);
...
bool Texture2D::SaveToTga(const std::string& vFilePathName, const bool& vFlipY, const int& vSubSamplesCount, const ez::uvec2& vNewSize) {
    ...
    if (vNewSize.x != width || vNewSize.y != height) {
        ...
        if (resizeRes) {
            if (vFlipY)
                stbi_flip_vertically_on_write(1);
            int resWrite = stbi_write_tga(vFilePathName.c_str(), newWidth, newHeight, bytesPerPixel, resizedData);
        ...
        if (vFlipY)
            stbi_flip_vertically_on_write(1);
        int resWrite = stbi_write_tga(vFilePathName.c_str(), width, height, bytesPerPixel, bmBytesRGBA);
...
bool Texture2D::UpdateMipMapping() {
    if (!m_VulkanCore.expired() && m_Texture2D != nullptr) {
        VulkanRessource::GenerateMipmaps(m_VulkanCore, m_Texture2D->image, m_ImageFormat, m_Width, m_Height, m_MipLevelCount);
        return true;
    }
    return false;
}
