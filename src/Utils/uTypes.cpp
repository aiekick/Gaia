...
GAIA_API bool IsTypeSplitable(const uTypeEnum& vType) {
    switch (vType) {
...
GAIA_API bool IsTypeCombinable(const uTypeEnum& vType) {
    switch (vType) {
        case uTypeEnum::U_FLOAT:
        case uTypeEnum::U_VEC2:
        case uTypeEnum::U_VEC3:
...
GAIA_API std::string ConvertUniformsTypeEnumToString(const uTypeEnum& vType) {
    switch (vType) {
...
GAIA_API uTypeEnum GetGlslTypeFromString(const std::string& vType, bool vIsArray) {
    if (vType == "text")
...
GAIA_API uTypeEnum GetBaseGlslTypeFromString(const std::string& vType, bool vIsArray, uint32_t* vCountChannels) {
    auto type = GetGlslTypeFromString(vType, vIsArray);
    return GetBaseGlslTypeFromType(type, vCountChannels);
}

GAIA_API uint32_t GetCountChannelForType(const uTypeEnum& vType) {
    uint32_t res = 0;
...
GAIA_API uTypeEnum GetBaseGlslTypeFromType(const uTypeEnum& vType, uint32_t* vCountChannels) {
